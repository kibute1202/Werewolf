...
import kh.nobita.hang.adapter.InGameWolfSniffingAdapter;
...
public class WolfSniffing extends Fragment implements OnClickListener {
    private String TAG = WolfSniffing.class.getSimpleName();
    private View view;
    ...
    private void initViews() {
        role = ListRoles.getInstance().getListRolesUsing().get(ListRoles.getInstance().getRoleInListShuffle(ListRoles.FLAG_WOLF_SNIFFING));
        ...
        rvPlayerSelect.setAdapter(new InGameWolfSniffingAdapter(getActivity(), ListPlayers.getInstance(getActivity()).getAllPlayerLive()));
