...
import kh.nobita.hang.adapter.InGameGrandmotherSelectPlayerAdapter;
...
public class Grandmother extends Fragment implements OnClickListener {
    private String TAG = Grandmother.class.getSimpleName();
    private View view;
    ...
    private InGameGrandmotherSelectPlayerAdapter adapterSelect;
    ...
    private void initViews() {
        role = ListRoles.getInstance().getListRolesUsing().get(ListRoles.getInstance().getRoleInListShuffle(ListRoles.FLAG_GRANDMOTHER));
        ...
        CircleImageView cvGrandmother = LayoutInit.createCircleView(getActivity());
        cvGrandmother.setImageResource(role.getPathImage());
        ...
        lnHistory.addView(cvGrandmother);
        ...
        adapterSelect = new InGameGrandmotherSelectPlayerAdapter(getActivity(), ListPlayers.getInstance(getActivity()).getListLimit(((GamePlay) getActivity()).getListPlayerGrandmotherSelect()));
        rvPlayerSelect.setAdapter(adapterSelect);
        rvPlayerSelect.setLayoutManager(myLayoutManagerPlayerSelect);
        ((GamePlay) getActivity()).getListPlayerGrandmotherSelect().clear();
                ...
                for (Player player : adapterSelect.getPlayerList()) {
                    if (player.isWolfBites()) {
                        if (((GamePlay) getActivity()).getNumberOldVillageWolfKill() < 2) {
                            if (!((GamePlay) getActivity()).checkRoleInverse(ListRoles.FLAG_GRANDMOTHER)) {
                                ((GamePlay) getActivity()).getListPlayerGrandmotherSelect().add(player);
                            }
                        }
