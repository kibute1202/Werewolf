...
import kh.nobita.hang.adapter.InGameHunterSelectPlayerAdapter;
...
public class Hunter extends Fragment implements OnClickListener {
    private String TAG = Hunter.class.getSimpleName();
    private View view;
    ...
    private InGameHunterSelectPlayerAdapter adapterSelect;
    ...
    private void initViews() {
        role = ListRoles.getInstance().getListRolesUsing().get(ListRoles.getInstance().getRoleInListShuffle(ListRoles.FLAG_HUNTER));
        ...
        adapterSelect = new InGameHunterSelectPlayerAdapter(getActivity(), ListPlayers.getInstance(getActivity()).getAllPlayerLive());
        ...
        if (role.getListPlayerLive().size() == 0) {
            ...
            ((GamePlay) getActivity()).getListPlayerDieWithHunter().clear();
        }
                ...
                for (Player player : adapterSelect.getPlayerList()) {
                    if (player.isWolfBites()) {
                        if (((GamePlay) getActivity()).getNumberOldVillageWolfKill() < 2) {
                            if (!((GamePlay) getActivity()).checkRoleInverse(ListRoles.FLAG_HUNTER)) {
                                if (player.getRole().getId() != ListRoles.FLAG_NOMADS || ((GamePlay) getActivity()).getPlayerNomads() == null) {
                                    ((GamePlay) getActivity()).getListPlayerDieWithHunter().add(player);
                                }
                            }
                        }
