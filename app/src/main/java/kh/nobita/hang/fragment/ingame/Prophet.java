...
import kh.nobita.hang.adapter.InGameProphetAdapter;
...
public class Prophet extends Fragment implements OnClickListener {
    private String TAG = Prophet.class.getSimpleName();
    private View view;
    ...
    private void initViews() {
        role = ListRoles.getInstance().getListRolesUsing().get(ListRoles.getInstance().getRoleInListShuffle(ListRoles.FLAG_PROPHET));
        ...
        rvPlayerSelect.setAdapter(new InGameProphetAdapter(getActivity(), ListPlayers.getInstance(getActivity()).getAllPlayerLive()));
