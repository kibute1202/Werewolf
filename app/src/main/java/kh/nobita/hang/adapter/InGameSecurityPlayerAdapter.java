...
public class InGameSecurityPlayerAdapter extends RecyclerView.Adapter<InGameSecurityPlayerAdapter.MyViewHolder> {
    ...
    public InGameSecurityPlayerAdapter(Context mContext, List<Player> playerList) {
...
//                .show();
    ...
    private void checkDie(int position) {
        ...
        if (playerList.get(position).isSecurity()) {
            ...
            if (getNumberSecurityHelp() < AppController.getInstance().getPrefManager().getNumberSecurityHelp()) {
                playerList.get(position).setSecurity(true);
            } else {
                new MaterialDialog.Builder(mContext)
                        .title(resources.getString(R.string.security_help_error_content))
                        .content(resources.getString(R.string.security_help_error_content) + " " + AppController.getInstance().getPrefManager().getNumberSecurityHelp())
                        .positiveText(resources.getString(R.string.agree))
                        .show();
            }
        }
    ...
    private int getNumberSecurityHelp() {
        int number = 0;
