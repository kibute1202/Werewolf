...
public class InGameNomadsSelectPlayerAdapter extends RecyclerView.Adapter<InGameNomadsSelectPlayerAdapter.MyViewHolder> {
    ...
    public InGameNomadsSelectPlayerAdapter(Context mContext, List<Player> playerList) {
...
//                .show();
    ...
    private void checkSelect(int position) {
        ...
        if (playerList.get(position).isWolfBites()) {
            ...
            if (getNumberSelect() < AppController.getInstance().getPrefManager().getNumberNomadsSelect()) {
                playerList.get(position).setWolfBites(true);
            } else {
                new MaterialDialog.Builder(mContext)
                        .title(resources.getString(R.string.hunter_select_error_title))
                        .content(resources.getString(R.string.hunter_select_error_content) + " " + AppController.getInstance().getPrefManager().getNumberNomadsSelect())
                        .positiveText(resources.getString(R.string.agree))
                        .show();
            }
        }
