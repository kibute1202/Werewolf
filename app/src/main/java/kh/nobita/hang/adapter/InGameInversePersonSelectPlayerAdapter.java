...
public class InGameInversePersonSelectPlayerAdapter extends RecyclerView.Adapter<InGameInversePersonSelectPlayerAdapter.MyViewHolder> {
    ...
    public InGameInversePersonSelectPlayerAdapter(Context mContext, List<Player> playerList) {
...
//                .show();
    ...
    private void checkSelect(int position) {
        Resources resources = LocaleHelper.getLangResources(mContext);
        if (playerList.get(position).isWolfBites()) {
            playerList.get(position).setWolfBites(false);
        } else {
            if (getNumberSelect() < AppController.getInstance().getPrefManager().getNumberInversePersonSelect()) {
                playerList.get(position).setWolfBites(true);
            } else {
                new MaterialDialog.Builder(mContext)
                        .title(resources.getString(R.string.hunter_select_error_title))
                        .content(resources.getString(R.string.hunter_select_error_content) + " " + AppController.getInstance().getPrefManager().getNumberInversePersonSelect())
                        .positiveText(resources.getString(R.string.agree))
                        .show();
            }
        }
