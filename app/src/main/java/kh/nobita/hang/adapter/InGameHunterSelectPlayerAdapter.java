...
public class InGameHunterSelectPlayerAdapter extends RecyclerView.Adapter<InGameHunterSelectPlayerAdapter.MyViewHolder> {
    ...
    public InGameHunterSelectPlayerAdapter(Context mContext, List<Player> playerList) {
...
//                .show();
    ...
    private void checkSelect(int position) {
        ...
        if (playerList.get(position).isWolfBites()) {
            ...
            if (getNumberSelect() < AppController.getInstance().getPrefManager().getNumberHunterSelect()) {
                playerList.get(position).setWolfBites(true);
            } else {
                new MaterialDialog.Builder(mContext)
                        .title(resources.getString(R.string.hunter_select_error_title))
                        .content(resources.getString(R.string.hunter_select_error_content) + " " + AppController.getInstance().getPrefManager().getNumberHunterSelect())
                        .positiveText(resources.getString(R.string.agree))
                        .show();
            }
        }
