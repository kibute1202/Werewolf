...
public class InGameProphetAdapter extends RecyclerView.Adapter<InGameProphetAdapter.MyViewHolder> {
    ...
    public InGameProphetAdapter(Context mContext, List<Player> playerList) {
    ...
    private void showRoleImfo(int position) {
        ...
        if (((GamePlay) mContext).checkRoleInverse(ListRoles.FLAG_PROPHET)) {
            isWolf = (isWolf ? false : true);
        }
        String content = "";
        if (isWolf) {
            content = resources.getString(R.string.prophet_show_wolf_content);
        } else {
            content = resources.getString(R.string.prophet_show_not_wolf_content);
        }
        new MaterialDialog.Builder(mContext)
                .title(resources.getString(R.string.prophet_show_tile))
                .content(content)
